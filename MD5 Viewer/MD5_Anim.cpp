...
void MD5Animation::InterpolateSkeletons( FrameSkeleton& finalSkeleton, const FrameSkeleton& skeleton0, const FrameSkeleton& skeleton1, float fInterpolate )
{
    for ( int i = 0; i < m_iNumJoints; ++i )
    {
        ...
        glm::mat4 orMat = glm::mat4_cast(finalJoint.m_Orient);
        glm::mat4 posMat = glm::translate(glm::mat4(), finalJoint.m_Pos);
        glm::mat4 mat = orMat * posMat;
        try{
        finalJoint.m_Orient = glm::normalize(finalJoint.m_Orient);
        }
        catch(int e)
        {
            std::cout << "Hubo un error" << e  << std::endl;
        }
    }
}
